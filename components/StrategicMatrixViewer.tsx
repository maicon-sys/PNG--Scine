...
import { StrategicMatrix } from '../types';
...
const ClarityMeter = ({ level }: { level: number }) => {
    const width = `${level}%`;
    let bgColor = 'bg-red-500';
    if (level > 40) bgColor = 'bg-yellow-500';
    if (level > 70) bgColor = 'bg-green-500';

    return (
        <div className="w-full bg-gray-200 rounded-full h-1.5 mt-1" title={`Nível de Clareza: ${level}%`}>
            ...
            <div className="p-4 space-y-2">
                <p className="text-xs text-gray-500 italic">{block.description}</p>
                 <ul className="space-y-1">
                    {block.items.map((i: any, index: number) => (
                        <li key={index} className="text-xs p-2 bg-white rounded border border-gray-100">
                           <strong className="font-semibold text-gray-800">{i.item}:</strong> {i.description}
                        </li>
                    ))}
                </ul>
            </div>
      ...
      {isOpen && (
          <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                  ...
                  <CanvasBlockDisplay title="Estrutura de Custos" block={matrix.costStructure} icon={<AlertTriangle className="w-4 h-á text-red-600"/>} />
              </div>
